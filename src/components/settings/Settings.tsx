...
import chooseTheDragonAction, {
  setLocalFriendDragon,
  setLocalAngryDragon,
} from '../../actions/chooseTheDragonAction';
...
const Settings = () => {
  ...
  const chooseMode = () => {
    dispatch(
      styleModeAction.toggleMode(
        `${mode === 'friendly' ? 'danger' : 'friendly'}`,
      ),
    );
  };
  ...
  return (
    <div className={style.settings}>
      <Button variant="contained" color={color} onClick={setLanguageToStorage}>
        <h5>{element.changeLang}</h5>
      </Button>
      <Button variant="contained" color={color} onClick={chooseMode}>
        <h5>{element.changeTheme}</h5>
      </Button>
      <div>
